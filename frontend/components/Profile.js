...
class Profile extends React.Component {
    ...
    onClickImagePress = async () => {                        // Function for image capture from camera and prediction
        ...
        if (!result.canceled) {
            ...
            this.setState({ image_src: result.assets[0].uri })
            let url = 'http://192.168.1.180/api/posts/';        //// Change the IP to with your own IP address
            console.log("???")
    ...
    onChooseImagePress = async () => {          // Function for selecting image from gallery and prediction
        ...
        if (!result.canceled) {
            let localUri = result.assets[0].uri;
            let form_data = new FormData();
            let url = 'http://192.1.1.1/api/posts/';   // Change the IP to with your own IP address
